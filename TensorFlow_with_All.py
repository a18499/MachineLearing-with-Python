...
# add hidden layer
l1 = add_layer(xs, 13, 13, activation_function=tf.nn.relu6)

# add hidden layer
l2 = add_layer(l1, 13, 10, activation_function=tf.nn.relu6)

# add output layer
prediction = add_layer(l2, 10, 1,  activation_function=tf.nn.sigmoid)
...
for i in range(1000):
    batch_xs, batch_ys = next_batch(200,docker_train,y_train)
   #dataset.train.n
    sess.run(train_step, feed_dict={xs: batch_xs, ys: batch_ys})
    if i % 100 == 0:
        print(compute_accuracy(docker_test, y_test))